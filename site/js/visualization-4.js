...
function IV_Highscool_Node() {
	...
	this.drawInit = function() {
		...
		textMale
			...
			.attr('dx', 5)
			.text(function(key) {
				return self.formatNumber(self.data[key].female)+' MÄNNER';
			});
		textMale
			.attr('y', function(key) {
				return self.getBarHeight(key)/2 + (d3.select(this).node().getBBox().height/2) - 4;
			...
			.each('end', function(key, i) {
				...
				if (key == self.keys[self.keys.length-1]) {
					if (Object.keys(self.child).length > 0) {
						...
						d3.selectAll('#'+self.idPrefix+self.currentKey+' > rect.pice_'+self.itemName)
							.transition()
							.duration(2000)
							.attr('height', function() { return self.heightBreadcrump; })
							.each('end', function(key, i) {

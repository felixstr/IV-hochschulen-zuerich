...
function IV_Highscool_Node() {
	...
	this.drawInit = function() {
		...
		enter
			...
			.text(function(key) {
				return self.data[key].female_percent+'% '+self.data[key].female+' FRAUEN';
			})
			.attr('x', 10)
			.attr('y', function(key) {
		...
		enter
			...
			.text(function(key) {
				return self.data[key].male_percent+'% '+self.data[key].male+' MÄNNER';
			})
			.attr('x', function(){
	...
	this.drawCurrent = function() {
		...
		var updateTransition = groupBar
			...
			.each('start', function(key, i) {
				...
				// d3.selectAll('#'+self.idPrefix+key).classed('active', true);
	...
	this.drawClosed = function(currentKey) {
		...
		*/
		...
		var updateTransition = groupBar
			...
			.each('end', function(key, i) {
				// console.log('end1');
				// d3.selectAll('#'+self.idPrefix+key).classed('active', false);
	...
	this.showBackLabel = function() {
		...
		if (self.yPosStartChild+self.heightActive < self.height) {
			...
			if (self.itemName == 'year') {
				...
				if (self.keys.length - self.barPositionTop.length > 1) {
	...
	*/		
	...
	this.showDetail = function() {
		this.container.selectAll('rect.bar_male, rect.bar_female').attr('style', 'display:none');
...
d3.json("data.json", function(data) {
	...
	// defs
	{
		...
		gradientFemale.append("svg:stop")
	    	.attr("offset", "0%")
			.attr("stop-color", "#940517")
			.attr("stop-opacity", 1);
		gradientFemale.append("svg:stop")
	    	.attr("offset", "100%")
			.attr("stop-color", "#e04244")
			.attr("stop-opacity", 1);
		...
		gradientMale.append("svg:stop")
	    	.attr("offset", "0%")
			.attr("stop-color", "#257dff")
			.attr("stop-opacity", 1);
		gradientMale.append("svg:stop")
	    	.attr("offset", "100%")
			.attr("stop-color", "#063462")
			.attr("stop-opacity", 1);
